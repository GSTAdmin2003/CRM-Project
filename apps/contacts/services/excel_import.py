"""
Excel parsing and import logic for Company import.
"""
...
def build_column_map(headers):
    ...
    Detection order:
    1. Company columns are detected ONLY until we hit the first unknown column
    ...
    # First pass: identify company columns
...
def validate_row(row_data, row_number):
    """
    Validate a single row's data for Company import.
...
def parse_excel_file(file_obj, user):
    """
    Parse an Excel file and validate its contents for company import.
    ...
    for row_idx, row in enumerate(ws.iter_rows(min_row=2), start=2):
        ...
        company_data = {
            ...
            'company_mobile': get_value_by_index(row_values, company_cols.get('company_mobile')),
        ...
        for group_num in sorted(column_map['contacts'].keys()):
            ...
            # Get contact name from the position column
...
def execute_import(preview_data, user):
    ...
    Creates Company and Contact records only (no leads).
            ...
            with transaction.atomic():
                ...
                try:
                    ...
                    if company_data.get('company_phone') and not company.company_phone:
                        ...
                        updated = True
                    if company_data.get('company_mobile') and not company.company_mobile:
                        company.company_mobile = company_data['company_mobile']
                        updated = True
                    ...
                    company = Company.objects.create(
                        ...
                        company_mobile=company_data.get('company_mobile', ''),
