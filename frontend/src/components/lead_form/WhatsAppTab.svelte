<script>
  ...
  export let lead = {};        // full lead object for contact phone
  export let apiUrls = {};
  ...
  $: messagesUrl = selectedConversation
    ? `${apiUrls.conversations}${selectedConversation.id}/messages/`
    : null;

  // Re-fetch conversations whenever the lead is updated (contact change, rep change, any save).
  ...
  async function fetchConversations() {
    loadingList = true;
    listError = '';
    // lead.contact is an integer FK — use the denormalized lead.phone field instead.
  ...
  function selectConversation(conv) {
    ...
    sendError = '';
    disconnectWs();
    _activeConvId = conv?.id ?? null;
    if (conv) {
      loading = true;
      const convId = conv.id;
      const url = `${apiUrls.conversations}${convId}/messages/`;
      fetchMessages(url).then(() => {
  ...
  async function sendNamedTemplate(templateName) {
    ...
    sendError = '';
    try {
      const url = `${apiUrls.conversations}${selectedConversation.id}/send_named_template/`;
    ...
    try {
      ...
      if (res.ok) {
        if (!ct.includes('application/json')) {
          ...
          console.error('[CRM:WhatsApp] fetchMessages: expected JSON, got', ct, 'from', url, '— final URL:', res.url, '— body starts:', text.slice(0, 300));
        ...
        const oldId = selectedConversation?.id;
        messages = [];
        selectedConversation = null;
        console.error('[CRM:WhatsApp] Conversation not found for id', oldId);
      } else {
        error = 'Failed to load messages';
  ...
  <div class="flex h-96 border border-gray-100 rounded-lg overflow-hidden">
    <div class="flex-1 flex flex-col overflow-hidden">
      ...
      {:else}
        <!-- Message list -->
        <div
          class="flex-1 overflow-y-auto space-y-2 p-3"
        ...
        >
          ...
          {:else if messages.length === 0}
            <div class="flex flex-col items-center justify-center h-full py-4 gap-3">
        ...
        <!-- Send error banner -->
        {#if sendError}
          <div class="mx-1 mb-1 px-3 py-1.5 bg-red-50 border border-red-200 rounded text-xs text-red-700 flex items-start gap-1.5">
